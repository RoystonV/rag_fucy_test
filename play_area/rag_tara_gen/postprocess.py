...
def print_summary(tara_json: dict) -> None:
    ...
    ap_count    = len(tara_json.get("attack_paths", {}).get("paths", []))
    cs_count    = len(tara_json.get("cybersecurity", {}).get("requirements", []))
    print(f"   Nodes              : {node_count}")
    print(f"   Edges              : {edge_count}")
    print(f"   Derivations        : {deriv_count}")
    print(f"   Damage details     : {ds_count}")
    print(f"   Attack paths       : {ap_count}")
    print(f"   Cybersec reqs      : {cs_count}")
