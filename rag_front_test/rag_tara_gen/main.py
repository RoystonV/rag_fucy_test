...
from db import save_report as db_save_report, is_connected as db_is_connected
...
Examples:
  ...
  python main.py --list-ecus
    ...
    # ── Save to MongoDB ───────────────────────────────────────────────────────
    print("\n[MongoDB] Saving report to tara_db.reports...")
    ecu_display = ecu_entry.get("name", user_query) if ecu_entry else user_query
    mongo_id = db_save_report(tara_json, query_name=user_query, ecu_name=ecu_display)
    if mongo_id:
        print(f"  ✅ MongoDB report saved (assets._id / doc id)")
    else:
        print("  ⚠️  MongoDB save skipped (check connection or install pymongo)")
